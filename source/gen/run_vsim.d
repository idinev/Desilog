...
void RunVSimOnVHDLFiles(string tbName){
	string res = tcl_format;

	string vhdlFiles;
	foreach(fname; vhdlAllGenFiles){
		vhdlFiles ~= "		" ~ fname ~ "\n";
	}

	string duration = format("%d", cfgTestBenchDuration);

	res = replace(res, "%{VHDL_FILES}", vhdlFiles);
	res = replace(res, "%{TESTBENCH}", tbName);
	res = replace(res, "${RUN_DURATION}", duration);

	string tclName = "run_" ~ tbName ~ ".tcl";
	std.file.write(tclName, res);
	std.stdio.writeln("Starting vsim -do ", tclName);
	shell("vsim -do " ~ tclName);
	std.stdio.writeln("Simulation complete");
}
...
set top_level  design_library.%{TESTBENCH}
set tb_duration ${RUN_DURATION}
